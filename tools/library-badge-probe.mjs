...
try {
  ...
  const r = await pg.evaluate(async () => {
    ...
    const cards = [...document.querySelectorAll('#sec-build .grid .card')].map((c) => ({ name: c.querySelector('span')?.textContent, title: c.title, chip: [...c.querySelectorAll('.opt-chip')].map((x) => x.textContent).join('') || null,
      ...
      box: (() => { const r = c.getBoundingClientRect(), row = c.querySelector('.opt-row')?.getBoundingClientRect(); return row ? { top: row.top - r.top, right: r.right - row.right } : null; })(),
  ...
  for (const [i, h] of r.json.entries()) {
    ...
    if (h.perf && !c.title.includes(`perf: ${h.perf.rankName}`)) mism.push(`${h.name}: hover lacks the rank`);
    if (c.box && (c.box.top < 4 || c.box.right < 4)) mism.push(`${h.name}: corner row inset ${JSON.stringify(c.box)}`);
  ...
  check('the corner row is inset ≥4px from the card edge and its chips are ≥3px apart', mism.filter((m) => /inset|touch/.test(m)).length === 0, mism.filter((m) => /inset|touch/.test(m)).slice(0, 3));
