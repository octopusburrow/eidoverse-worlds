...
try {
  ...
  const r = await pg.evaluate(async () => {
    ...
    const cards = [...document.querySelectorAll('#sec-build .grid .card')].map((c) => ({ name: c.querySelector('span')?.textContent, title: c.title, chip: c.querySelector('.opt-chip')?.textContent ?? null }));
    return { json, cards };
  });
  ...
  for (const [i, h] of r.json.entries()) {
    ...
    const want = bad ? '⚠' : h.opt.lod?.state === 'built' ? 'LOD' : null;
    if (c.chip !== want) mism.push(`${h.name}: chip ${c.chip} want ${want}`);
    counts[want === '⚠' ? 'warn' : want === 'LOD' ? 'lod' : 'none']++;
