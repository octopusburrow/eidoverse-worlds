...
try {
  ...
  await sleep(700);   // the fold echoes
  const ins = await rpc('tools/call', { name: 'inspect', arguments: { id: 'lamp1' } });
  ...
  await sleep(700);
  const ins2 = await rpc('tools/call', { name: 'inspect', arguments: { id: 'lamp1' } });
  ...
  check('a field a light does not have is refused by name, nothing sent', /refused: pos\.yaw/.test(toolText(bad)) && !/sent/.test(toolText(bad)), toolText(bad));
