...
const r = await page.evaluate(async () => {
  ...
  await feed(0, 0, 1500);
  const s0 = { hipsY: +(xb.xrBodyDebug().hipsYaw ?? 0).toFixed(3), rootY: +me.root.rotation.y.toFixed(3), rigY: +rig.rotation.y.toFixed(3), eyeErr: +eyeW().distanceTo(new T.Vector3(0.3, 1.42, -0.2)).toFixed(3), sceneP: me.vrm.scene.position.toArray().map(v => +v.toFixed(3)) };
  await feed(1.0, 0, 1800);
  const s1 = { hipsY: +(xb.xrBodyDebug().hipsYaw ?? 0).toFixed(3), rootY: +me.root.rotation.y.toFixed(3), rigY: +rig.rotation.y.toFixed(3), eyeErr: +eyeW().distanceTo(new T.Vector3(0.3, 1.42, -0.2)).toFixed(3), restYaw: me.vrm.scene.userData.restYaw ?? null, follow: xb.xrBodyDebug().follow ?? null };
  ...
  const s2 = { hipsY: +(xb.xrBodyDebug().hipsYaw ?? 0).toFixed(3), scenePY: +me.vrm.scene.position.y.toFixed(3), eyeErr: +eyeW().distanceTo(new T.Vector3(0.3, 1.42, -0.2)).toFixed(3), headFwdY: +hf.y.toFixed(3), headUp: hf.y > 0.1 };
